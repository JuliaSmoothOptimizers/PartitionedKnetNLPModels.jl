...
function LSCG_AdaN(nlp :: AbstractNLPModel, B :: AbstractLinearOperator{T};
  ...
  β = -1.
    ...
    Krylov.cg!(subsolver, B, gₜₘₚ, atol=T(atol), rtol=cgtol, linesearch=true)
		...
		if linesearch 
      (β, fₖ₊₁) = linesearch_AdaN!(x, fₖ, sₖ, nlp, gₖ; vec=xtmp, α) # we compute the ratio    
    else
      β = 1.
      x .= x .+ β .* sₖ
    end
...
"""
    linesearch_AdaN!x::Vector{T}, fₖ::T, sₖ::Vector{T}, nlp::AbstractNLPModel{T}, gₖ::AbstractVector{T}; vec=similar(x), c=0.05, τ=2/3, init=1., β = init, α=0.05 ) where T <: Number

Find the step length β.
"""
function linesearch_AdaN!(x::Vector{T},
  fₖ::T,
  sₖ::Vector{T},
